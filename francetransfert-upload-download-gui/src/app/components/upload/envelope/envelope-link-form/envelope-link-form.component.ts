...
export class EnvelopeLinkFormComponent implements OnInit, OnDestroy {
  ...
  initForm() {
    this.envelopeLinkForm = this.fb.group({
      ...
      from: [
        this.loginService.tokenInfo.getValue() &&
      ...
      message: [
        this.linkFormValues?.message,
        { validators: [Validators.maxLength(2500)] },
      ],
      cguCheck: [
        this.linkFormValues?.cguCheck,
        { validators: [Validators.requiredTrue] },
      ],
    });
    this.envelopeLinkFormStatusSubscription =
      this.envelopeLinkForm.statusChanges.subscribe(() => {
        let mailSend = this.envelopeLinkForm
          .get('from')
          .value.toLowerCase()
          .trim();
        if (mailSend != this.envelopeLinkForm.get('from').value) {
          this.envelopeLinkForm
            .get('from')
            .setValue(mailSend, { emitEvent: true, onlySelf: false });
        }
      });
    this.envelopeLinkFormChangeSubscription =
      this.envelopeLinkForm.valueChanges.subscribe(() => {
        this.checkEmitter();
        this.uploadManagerService.envelopeInfos.next({
          type: 'link',
          ...this.envelopeLinkForm.value,
          ...(this.uploadManagerService.envelopeInfos.getValue()?.parameters
            ? {
                parameters:
                  this.uploadManagerService.envelopeInfos.getValue().parameters,
              }
    ...
    this.loginSubscription = this.loginService.loggedIn$.subscribe(loggedIn => {
      if (loggedIn) {
        this.envelopeLinkForm
          .get('from')
          .patchValue(this.loginService.tokenInfo.getValue().senderMail);
      }
    });
  }
  ...
  get f() {
    return this.envelopeLinkForm.controls;
  }
  ...
  checkEmitter() {
    // In E2EE mode the email domains whitelist check does not apply (the link
    // carries the decryption key and is meant to be shared freely). We only
    // keep the email format + send-quota checks.
    const encryptionEnabled =
      this.uploadManagerService.encryptionEnabled.getValue();
    let senderOk = false;
    this.uploadService
      .validateMail([this.envelopeLinkForm.get('from').value])
      .pipe(take(1))
      .subscribe(
        (isValid: boolean) => {
          senderOk = encryptionEnabled ? true : isValid;
          if (this.envelopeLinkForm.get('from').value) {
            if (!senderOk) {
              this.envelopeLinkForm.controls['from'].markAsTouched();
              this.envelopeLinkForm.controls['from'].setErrors({
                notValid: true,
              });
              this.onFormGroupChange.emit({
                isValid: this.envelopeLinkForm.valid,
                values: this.envelopeLinkForm.value,
              });
            } else {
              this.envelopeLinkForm.controls['from'].markAsTouched();
              this.envelopeLinkForm.controls['from'].setErrors(null);
              this.uploadService
                .allowedSenderMail(this.envelopeLinkForm.get('from').value)
                .pipe(take(1))
