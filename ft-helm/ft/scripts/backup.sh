...
delete_date=$(date -d @$(($(date +%s) - 86400 * 7)) +%Y-%m-%d)
echo "clean old redis dump in s3"
aws --no-verify-ssl s3api list-objects --endpoint-url $STORAGE_ENDPOINT --bucket $BACKUP_BUCKET_NAME --query 'Contents[?LastModified<=`'$delete_date'`].{Key:Key}' --output text | xargs -I {} aws s3 rm s3://$BACKUP_BUCKET_NAME/{} --endpoint-url $STORAGE_ENDPOINT
echo "finish backup"