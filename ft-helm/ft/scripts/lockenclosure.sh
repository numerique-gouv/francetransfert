...
if [ -z "${expired_timestamp}" ]; then
  ...
  if [ -n "${now_epoch}" ]; then
    past_epoch=$((now_epoch - 30 * 24 * 60 * 60))
    expired_timestamp=$(date -u -d "@${past_epoch}" +"%Y-%m-%dT%H:%M" 2>/dev/null || true)
    if [ -z "${expired_timestamp}" ]; then
      expired_timestamp=$(date -u -r "${past_epoch}" +"%Y-%m-%dT%H:%M" 2>/dev/null || true)
    fi
  fi
fi
if [ -z "${expired_timestamp}" ]; then
